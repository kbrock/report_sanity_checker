...
class ReportSanityChecker
  ...
  def check_report(filename)
    ...
    if column_report
      if filename.kind_of?(MiqReport)
        # No header for MiqReport objects
      else
        name = filename
  ...
  def print_details(rpt)
    ...
    includes_cols = rpt.includes_columns
    ...
    # Get condition and display filter columns
    cond_cols = rpt.condition_columns
    display_cols = rpt.display_filter_columns
    ...
    tbl = Table.new
    visible_headers = HEADERS.each_with_index.select { |_, i| HEADERS_SHOW[i] }.map(&:first)
    tbl.headings = visible_headers

    # Add columns to the table
    print_cols_to_table(tbl, rpt, rpt.col_order, "hidden", includes_cols, cond_cols, display_cols)
    ...
    sql_only = Set.new(rpt.cols) - rpt.col_order
    print_cols_to_table(tbl, rpt, sql_only, "sql only", includes_cols, cond_cols, display_cols)
    ...
    print_cols_to_table(tbl, rpt, include_only, "include", includes_cols, cond_cols, display_cols)
    ...
    sort_only = Set.new(Array.wrap(rpt.sortby)) - rpt.col_order - includes_cols - Set.new(rpt.cols)
    print_cols_to_table(tbl, rpt, sort_only, "sort only", includes_cols, cond_cols, display_cols)

    # for these: need to convert reports to using Field vs target...
    cond_only = cond_cols - rpt.col_order - includes_cols - Set.new(rpt.cols)
    print_cols_to_table(tbl, rpt, cond_only, "cond only", includes_cols, cond_cols, display_cols)
  ...
  def run_report(rpt, options = {:limit => @limit})
  ...
  def print_cols_to_table(table, rpt, cols, desc, includes_cols, cond_cols, display_cols)
    klass = rpt.db_class
    rpt_cols = Set.new(rpt.cols)
    col_order = rpt.col_order
    ...
    cols.each do |col|
      ...
      *class_names, col_name = [klass&.name || "unknown"] + col.split(".")
      ...
      # Determine virtual attribute type
      va =
      ...
      # Determine SQL support
      # TODO: add MiqExpression::Tag#attribute_supported_by_sql?
      sql_support = klass ? f.try(:attribute_supported_by_sql?) ? "sql" : "ruby" : "?"

      # Create row values and add to table
      values = [col, "", va, sql_support, col_src, is_alias, in_sort, in_col, in_miq]
      visible_values = values.each_with_index.select { |_, i| HEADERS_SHOW[i] }.map(&:first)
      table << visible_values
    end
  end
